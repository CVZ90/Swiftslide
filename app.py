...
# 4. واجهة المستخدم
topic = st.text_input("What is your topic?", placeholder="e.g. Modern Architecture")
    ...
    if topic:
        with st.spinner('AI is generating content...'):
            try:
                ...
                # استخدام الاسم المختصر للنموذج (بدون models/)
                # هذا الاسم مأخوذ مباشرة من القائمة التي ظهرت لك
                ...
                # إذا ظهر خطأ الـ 404 مجدداً، سنستخدم اسماً بديلاً فوراً
                if "404" in str(e):
                    st.warning("Retrying with alternative model name...")
                    try:
                        model = genai.GenerativeModel('gemini-pro')
                        response = model.generate_content(f"Summarize {topic} in 5 slides.")
                        st.write(response.text)
                    except:
                        st.error("Model not found. Please check your API Key region.")
                else:
